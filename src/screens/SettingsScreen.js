...
function SettingTouchable(props) {
    return (
        <TouchableHighlight
            onPress={() => props.onPress()} underlayColor='#b2dfdc'
        >
            <View style={[
                {
                    marginLeft: 16,
...
function SelectableFlatList(props) {
    ...
    return (
        <FlatList
            initialScrollIndex={props.selectedItem ? props.data.findIndex(el => el === props.selectedItem) : 0}
...
export default function SettingsScreen({ navigation }) {
    ...
    const categories = ['none'].concat(...Arxiv.categories.map(sect => sect.data.map(cat => cat.category)));
    ...
    return (
        <View style={styles.container}>
            <SelectableFlatListModal
                ...
                onSelect={(item) => {
                    updateConfig('defaultCategory', item === 'none' ? undefined : item)
